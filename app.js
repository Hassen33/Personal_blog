...
// getting parameter
app.get('/posts/:reqPostTiltle', (req,res) => {
  posts.filter((p) => {
    
    const pstTtitle = _.lowerCase(p.postTitle);
    const postContent = p.post;
    const reqPostTiltle = _.lowerCase(req.params.reqPostTiltle);
    if (pstTtitle == reqPostTiltle) {
      res.render('post', {
        requestedTitleKey: pstTtitle,
